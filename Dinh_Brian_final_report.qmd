...
Before exploring this question, I wanted to check to see the different types of buildings being sold in Brooklyn, because my initial hypothesis assumes that the building type can greatly affect certain data about the building. The tax class at sale categorizes the type of building sold into 4 categories according to the state of New York:
...
Another variable I wanted to look at was the proximity code, which identifies the proximity of the property to another property. `prox_code` is split into three categories: detached, semi-attached, and attached. My initial assumption is that detached homes are the most popular due to having more privacy.
