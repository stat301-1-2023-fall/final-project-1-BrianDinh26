...
brooklyn_sales_nonres |> 
  ...
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
```
(add a more thorough explanation with a segue into next graph here later. tldr my hypothesis was wrong, less sales were in a time of cheaper buildings? why so? let's look year by year for the reasons why or at least explanationish)
```{r}
...
```
(general tldr is that for the 2007-2010 time period the most popular homes were still the 400kish range ones, it's just that there were significantly less homes sold at higher prices)

## What neighborhoods are the most popular in Brooklyn?

(info introducing how many neighborhoods brooklyn has. give a map (externally sourced need to cite in the references) showing a map of neighborhoods).

*start with table that sorts by count sold. shows that top 10 only has park slope as common in both most expensive and most popular. 
```{r}
brooklyn_sales_res |> 
  ...
  DT::datatable(
    ...
    caption = htmltools::tags$caption(
      ...
      'Table: ', htmltools::em('Brooklyn Neighborhoods Ranked by Count of Buildings Sold, 2003-2017.'))
  ) |> 
...
*then go with figure with 3 barplots showing off neighborhood popualrity over 3 time periods. tldr is that they don't change very much, sameish 3/4 neighborhoods dominate the top 5.)
